...
const initialConversations: Conversation[] = [
  {
    ...
    time: "9:42",
    messages: [
      { id: 1, body: "Are we still meeting at the café tomorrow?", sender: "them" },
      { id: 2, body: "Yes, 10 works for me.", sender: "me" },
      { id: 3, body: "Perfect. See you tomorrow.", sender: "them" },
    ],
  },
  {
    id: 2,
    ...
    time: "8:18",
    messages: [
      { id: 1, body: "I pushed the update.", sender: "them" },
      { id: 2, body: "I’ll take a look this morning.", sender: "me" },
    ],
  },
  {
    id: 3,
    ...
    time: "Yesterday",
    messages: [
      { id: 1, body: "Want to grab lunch this week?", sender: "me" },
      { id: 2, body: "Sounds good.", sender: "them" },
    ],
  },
];
    ...
    <main className="h-dvh bg-stone-100 p-0 text-stone-900 sm:p-4">
      <div className="relative mx-auto grid h-full max-w-6xl overflow-hidden border-stone-300 bg-white sm:grid-cols-[280px_1fr] sm:rounded-lg sm:border">
        ...
        <aside className={`absolute inset-y-0 left-0 z-20 flex w-[280px] flex-col border-r border-stone-200 bg-white transition-transform sm:static sm:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
          ...
          <nav aria-label="Conversations" className="flex-1 overflow-y-auto px-2">
            {visibleConversations.map((conversation) => {
              ...
              return (
                <button className={`flex w-full items-center gap-3 rounded-md px-3 py-3 text-left ${conversation.id === activeId ? "bg-stone-100" : "hover:bg-stone-50"}`} key={conversation.id} onClick={() => chooseConversation(conversation.id)} type="button">
        ...
        <section className="flex min-w-0 flex-col">
          ...
          <form className="flex shrink-0 items-end gap-2 border-t border-stone-200 p-3 sm:p-4" onSubmit={sendMessage}>
            ...
            <textarea className="max-h-32 min-h-10 flex-1 resize-none rounded-lg border border-stone-300 px-3 py-2 text-sm leading-6 outline-none placeholder:text-stone-400 focus:border-stone-500" id="message" onChange={(event) => setDraft(event.target.value)} placeholder="Write a message" rows={1} value={draft} />

...
export function ChatShell({
  ...
  useEffect(() => {
    const announcePresence = () => void fetch("/api/presence", { method: "POST" });
    announcePresence();
    const heartbeat = setInterval(announcePresence, 25000);
            ...
            <div className="space-y-4">
              {activeConversation.messages.map((message) => (
                <div className={`flex rounded-xl transition ${messageMatches[activeMatch] === message.id ? "bg-amber-50/70" : ""} ${message.sender === "me" ? "justify-end" : "justify-start"}`} id={`message-${message.id}`} key={message.id}>
                  ...
                  <div className="ml-2 flex shrink-0 flex-col items-end justify-end gap-1 pb-5 text-[10px] text-stone-400">{message.attachment?.type.startsWith("audio/") && <audio className="h-8 w-52" controls preload="metadata" src={message.attachment.url} />}<span className="flex gap-1"><button className="rounded px-1.5 py-1 hover:bg-stone-100 hover:text-stone-900" onClick={() => void pinMessage(message)} type="button">{message.pinned ? "Unpin" : "Pin"}</button><button className="rounded px-1.5 py-1 hover:bg-stone-100 hover:text-stone-900" onClick={() => setForwardingMessage(message)} type="button">Forward</button>{message.pinned && <span className="rounded bg-stone-100 px-1.5 py-1 text-stone-600">Pinned</span>}</span></div>
                </div>
              ))}

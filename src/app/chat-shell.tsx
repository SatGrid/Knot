...
function VoiceMessage({ attachment, outgoing }: { attachment: NonNullable<MessageView["attachment"]>; outgoing: boolean }) {
  ...
  return <div className={`my-0.5 flex min-w-[250px] max-w-[320px] items-center gap-2.5 ${outgoing ? "text-white" : "text-stone-800"}`}>
    <audio ref={audioRef} onEnded={() => setPlaying(false)} onLoadedMetadata={(event) => setDuration(event.currentTarget.duration)} onPause={() => setPlaying(false)} onPlay={() => setPlaying(true)} onTimeUpdate={(event) => setCurrentTime(event.currentTarget.currentTime)} preload="metadata" src={attachment.url} />
    <button aria-label={playing ? "Pause voice message" : "Play voice message"} className={`grid size-9 shrink-0 place-items-center rounded-full ${outgoing ? "bg-white text-stone-900" : "bg-stone-900 text-white"}`} onClick={togglePlayback} type="button">{playing ? <span className="flex gap-1"><i className="h-3.5 w-1 rounded-sm bg-current"/><i className="h-3.5 w-1 rounded-sm bg-current"/></span> : <span className="ml-0.5 block h-0 w-0 border-y-[7px] border-l-[11px] border-y-transparent border-l-current"/>}</button>
    <div className="min-w-0 flex-1"><input aria-label="Voice message progress" className="block h-1.5 w-full cursor-pointer accent-current" max={duration || 0} min="0" onChange={(event) => { const audio = audioRef.current; if (audio) audio.currentTime = Number(event.target.value); }} step="0.1" type="range" value={Math.min(currentTime, duration || 0)} /><span className={`mt-1 block text-[10px] tabular-nums ${outgoing ? "text-stone-300" : "text-stone-500"}`}>{formatAudioTime(currentTime)} / {formatAudioTime(duration)}</span></div>
    <button aria-label={`Playback speed ${speed} times`} className={`min-w-8 rounded-md px-1.5 py-1 text-[10px] font-semibold tabular-nums ${outgoing ? "bg-white/15 hover:bg-white/25" : "bg-black/5 hover:bg-black/10"}`} onClick={cycleSpeed} type="button">{speed}×</button>
    <a aria-label="Download voice message" className={`grid size-8 shrink-0 place-items-center rounded-full ${outgoing ? "hover:bg-white/15" : "hover:bg-black/5"}`} download={attachment.name} href={attachment.url} title="Download voice message"><svg aria-hidden="true" className="size-4" fill="none" viewBox="0 0 24 24"><path d="M12 3v12m0 0 4-4m-4 4-4-4M5 20h14" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8"/></svg></a>
  </div>;
}
...
export function ChatShell({
  ...
  const [viewingProfile, setViewingProfile] = useState<ConversationView | null>(null);
  ...
  const [presenceStatuses, setPresenceStatuses] = useState<Record<string, string>>({});
  ...
  const [conversations, applyOptimisticChange] = useOptimistic(
    ...
    (current, change: OptimisticChange) => current.map((conversation) => {
      if (change.type === "add" && conversation.id === change.conversationId) {
        return { ...conversation, time: "Now", messages: [...conversation.messages, { id: change.id, body: change.body, sender: "me", time: "Now", delivery: "Sent", edited: false, replyTo: change.replyTo, reactions: [], attachment: change.attachment, pinned: false }] };
      }
      if (change.type === "edit") {
  ...
  useEffect(() => {
    ...
    events.onmessage = (message) => {
      const event = JSON.parse(message.data) as { type?: string; conversationId: string; userName?: string; userId?: string; isTyping?: boolean; isOnline?: boolean; senderName?: string; messagePreview?: string };
      ...
      if (event.type === "presence" && event.userId) {
        const matchingIds = (participantConversationIds.current[event.userId] ?? []).map(({ id }) => id);
        setPresenceStatuses((current) => ({ ...current, ...Object.fromEntries(matchingIds.map((id) => [id, "Online"])) }));
        const existing = presenceTimers.current.get(event.userId);
        if (existing) clearTimeout(existing);
        presenceTimers.current.set(event.userId, setTimeout(() => setPresenceStatuses((current) => ({ ...current, ...Object.fromEntries(matchingIds.map((id) => [id, "Last seen just now"])) })), 55000));
        return;
      }
      if (notificationsEnabled && event.messagePreview && document.hidden && Notification.permission === "granted") {
        new Notification(event.senderName || "New message", { body: event.messagePreview, tag: event.conversationId });
      }
      router.refresh();
    };
    return () => events.close();
  ...
  useEffect(() => {
    ...
    const announcePresence = async () => {
      ...
      requestRunning = true;
      try { await fetch("/api/presence", { method: "POST" }); } finally { requestRunning = false; }
    };
    void announcePresence();
    const heartbeat = setInterval(() => void announcePresence(), 45000);
    const onVisibilityChange = () => { if (!document.hidden) void announcePresence(); };
    document.addEventListener("visibilitychange", onVisibilityChange);
    return () => { clearInterval(heartbeat); document.removeEventListener("visibilitychange", onVisibilityChange); };
  }, []);
  ...
  useEffect(() => {
    ...
    const storedNotifications = window.localStorage.getItem("knot-notifications") === "on" && Notification.permission === "granted";
    queueMicrotask(() => { setDarkMode(storedDarkMode); setDraft(storedDraft); setNotificationsEnabled(storedNotifications); });
  }, []);
  ...
  async function toggleNotifications() {
    if (notificationsEnabled) {
      ...
      window.localStorage.setItem("knot-notifications", "off");
    } else {
      const permission = await Notification.requestPermission();
      const enabled = permission === "granted";
      setNotificationsEnabled(enabled);
      window.localStorage.setItem("knot-notifications", enabled ? "on" : "off");
    }
    setMenuOpen(false);
  }
  ...
  async function toggleRecording() {
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        void uploadAttachment(new File([blob], `Voice note ${new Date().toLocaleTimeString()}.webm`, { type: blob.type }));
      ...
      setAttachmentError("Microphone access is needed to record a voice note.");
    }
  }

  async function pinMessage(message: MessageView) {
  ...
  async function forwardTo(conversationId: string) {
    ...
    await forwardMessage(forwardingMessage.id, conversationId);
  ...
  async function blockContact() {
    ...
    await toggleBlockUser(activeConversation.participantId);
    setMenuOpen(false);
    router.refresh();
  }
  ...
  async function confirmRemoveConversation() {
    ...
    setRemoveConversationOpen(false);
    router.refresh();
  }
  ...
  if (!activeConversation) {
    return (
      <main className={`${darkMode ? "knot-dark" : ""} knot-app h-dvh overflow-hidden p-0 text-slate-900 sm:p-5`}>
        ...
        <div className="knot-empty-shell knot-shell mx-auto grid h-full max-w-7xl overflow-hidden sm:grid-cols-[292px_1fr] sm:rounded-[22px]">
  ...
  return (
    <main className={`${darkMode ? "knot-dark" : ""} knot-app h-dvh overflow-hidden p-0 text-slate-900 sm:p-5`}>
      {introVisible && <KnotIntro />}
      <div className={`knot-shell relative mx-auto grid h-full max-w-7xl overflow-hidden sm:rounded-[22px] ${sidebarCollapsed ? "is-sidebar-collapsed sm:grid-cols-[0px_1fr]" : "sm:grid-cols-[292px_1fr]"}`}>
        ...
        <section className="knot-chat flex min-h-0 min-w-0 flex-col overflow-hidden bg-white">
          <header className="knot-chat-header flex h-[68px] shrink-0 items-center justify-between border-b px-4 sm:px-6">
            <div className="flex items-center gap-3">
              ...
              <button aria-label={`View ${activeConversation.name}'s profile`} className="flex items-center gap-3 rounded-xl p-1 text-left transition hover:bg-stone-100" onClick={() => setViewingProfile(activeConversation)} type="button">
          ...
          <div aria-live="polite" className="knot-messages min-h-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-contain px-3 py-7 sm:px-5">
            <div className="flex min-h-full flex-col justify-end gap-4">
              ...
              <div className="h-px shrink-0" ref={messageEndRef} />
            </div>
          </div>
          ...
          <form className="knot-composer flex shrink-0 items-end gap-2 border-t p-3 sm:px-5 sm:py-4" onSubmit={(event) => { event.preventDefault(); void submitMessage(new FormData(event.currentTarget)); }}>
            ...
            <button aria-label={recording ? "Stop recording" : "Record voice note"} className={`knot-compose-action grid size-10 shrink-0 place-items-center rounded-xl ${recording ? "is-recording bg-stone-900 text-white" : "text-stone-500"}`} disabled={activeConversation.blocked} onClick={() => void toggleRecording()} title={recording ? "Stop recording" : "Record voice note"} type="button">{recording ? <span className="size-3 rounded-sm bg-current" /> : <svg aria-hidden="true" className="size-[18px]" fill="none" viewBox="0 0 24 24"><path d="M12 15.5a4 4 0 0 0 4-4V7a4 4 0 1 0-8 0v4.5a4 4 0 0 0 4 4Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8"/><path d="M5.5 11.5a6.5 6.5 0 0 0 13 0M12 18v3M9 21h6" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8"/></svg>}</button>
      ...
      {viewingProfile && <div className="fixed inset-0 z-40 grid place-items-center bg-stone-950/35 px-4 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) setViewingProfile(null); }}><section className="knot-contact-profile w-full max-w-sm overflow-hidden rounded-3xl border border-white/60 bg-white shadow-2xl shadow-black/20"><div className="relative flex flex-col items-center bg-gradient-to-b from-stone-100 to-white px-6 pb-6 pt-8"><button aria-label="Close profile" className="absolute right-4 top-4 grid size-8 place-items-center rounded-full bg-white/80 text-lg text-stone-500 shadow-sm hover:bg-white" onClick={() => setViewingProfile(null)} type="button">×</button>{viewingProfile.avatarUrl ? <button aria-label={`Open ${viewingProfile.name}'s profile picture`} className="rounded-full transition hover:scale-[1.02] focus:outline-none focus:ring-4 focus:ring-stone-300" onClick={() => setViewingImage({ url: viewingProfile.avatarUrl!, name: `${viewingProfile.name}'s profile picture`, type: "image/profile", size: 0 })} title="Open profile picture" type="button"><Avatar className="size-28 text-3xl shadow-lg ring-4 ring-white" name={viewingProfile.name} url={viewingProfile.avatarUrl} /></button> : <Avatar className="size-28 text-3xl shadow-lg ring-4 ring-white" name={viewingProfile.name} />}<h2 className="mt-4 text-xl font-semibold tracking-tight text-stone-900">{viewingProfile.name}</h2>{viewingProfile.username && <p className="mt-1 text-sm text-stone-500">@{viewingProfile.username}</p>}<p className="mt-2 flex items-center gap-1.5 text-xs text-stone-500"><span className={`size-1.5 rounded-full ${(presenceStatuses[viewingProfile.id] ?? viewingProfile.status) === "Online" ? "bg-emerald-500" : "bg-stone-300"}`} />{presenceStatuses[viewingProfile.id] ?? viewingProfile.status}</p></div><div className="border-t border-stone-100 px-6 py-5">{viewingProfile.email && <div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-stone-400">Email</p><p className="mt-1 text-sm text-stone-700">{viewingProfile.email}</p></div>}<button className="mt-5 w-full rounded-xl bg-stone-900 px-4 py-2.5 text-sm font-medium text-white hover:bg-stone-700" onClick={() => setViewingProfile(null)} type="button">Back to chat</button></div></section></div>}
      {profileOpen && <div className="fixed inset-0 z-30 grid place-items-center bg-stone-950/30 px-4 backdrop-blur-[2px]" onMouseDown={(event) => { if (event.target === event.currentTarget) setProfileOpen(false); }}><form className="knot-profile-editor w-full max-w-md rounded-2xl border border-stone-200 bg-white p-6 shadow-2xl" onSubmit={(event) => { event.preventDefault(); void saveProfile(); }}><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold tracking-tight">Your profile</h2><p className="mt-1 text-sm text-stone-500">How people see you on Knot.</p></div><button aria-label="Close profile" className="grid size-8 place-items-center rounded-full bg-stone-100 text-stone-500 hover:bg-stone-200" onClick={() => setProfileOpen(false)} type="button">×</button></div><div className="mt-6 flex items-center gap-4">{profileAvatar ? <button aria-label="Open your profile picture" className="rounded-full transition hover:scale-[1.02] focus:outline-none focus:ring-4 focus:ring-stone-300" onClick={() => setViewingImage({ url: profileAvatar, name: "Your profile picture", type: "image/profile", size: 0 })} title="Open profile picture" type="button"><Avatar className="size-20 text-xl" name={profileName || currentUserName} url={profileAvatar} /></button> : <Avatar className="size-20 text-xl" name={profileName || currentUserName} />}<div><label className="inline-flex cursor-pointer rounded-lg border border-stone-300 px-3 py-2 text-sm font-medium hover:bg-stone-50">Choose photo<input accept="image/*" className="sr-only" onChange={(event) => chooseProfilePhoto(event.target.files?.[0])} type="file" /></label>{profileAvatar && <button className="ml-2 px-2 py-2 text-sm text-stone-500 hover:text-stone-900" onClick={() => setProfileAvatar("")} type="button">Remove</button>}</div></div><div className="mt-6 space-y-4"><label className="block text-sm"><span className="mb-1.5 block font-medium">Display name</span><input className="h-10 w-full rounded-lg border border-stone-300 bg-stone-50 px-3 outline-none focus:border-stone-500" maxLength={50} minLength={2} onChange={(event) => setProfileName(event.target.value)} required value={profileName} /></label><label className="block text-sm"><span className="mb-1.5 block font-medium">Username</span><div className="flex h-10 items-center rounded-lg border border-stone-300 bg-stone-50 px-3 focus-within:border-stone-500"><span className="text-stone-400">@</span><input className="min-w-0 flex-1 bg-transparent outline-none" maxLength={20} minLength={3} onChange={(event) => setProfileUsername(event.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ""))} placeholder="your_username" value={profileUsername} /></div></label></div>{profileError && <p className="mt-3 text-sm text-red-600">{profileError}</p>}<div className="mt-6 flex justify-end gap-2"><button className="rounded-lg px-4 py-2.5 text-sm font-medium text-stone-500 hover:bg-stone-100" disabled={profilePending} onClick={() => setProfileOpen(false)} type="button">Cancel</button><button className="rounded-lg bg-stone-900 px-4 py-2.5 text-sm font-medium text-white disabled:bg-stone-300" disabled={profilePending || profileName.trim().length < 2} type="submit">{profilePending ? "Saving…" : "Save profile"}</button></div></form></div>}
      ...
      {viewingImage && <div className={`knot-image-viewer fixed inset-0 z-50 flex flex-col bg-black/65 text-white backdrop-blur-xl ${viewingImage.type === "image/profile" ? "is-profile" : ""}`} onMouseDown={(event) => { if (event.target === event.currentTarget) setViewingImage(undefined); }}><header className="flex h-16 shrink-0 items-center justify-between px-4 sm:px-6"><button className="rounded-lg px-3 py-2 text-sm font-medium hover:bg-white/10" onClick={() => setViewingImage(undefined)} type="button">← Back</button><p className="max-w-[60%] truncate text-sm text-stone-300">{viewingImage.name}</p><button aria-label="Close image" className="grid size-10 place-items-center rounded-full text-2xl hover:bg-white/10" onClick={() => setViewingImage(undefined)} type="button">×</button></header><div className="knot-image-stage relative mx-auto min-h-0 w-full flex-1 p-4 sm:p-8"><Image alt={viewingImage.name} className="object-contain drop-shadow-2xl" fill priority sizes="100vw" src={viewingImage.url} unoptimized /></div></div>}
      {forwardingMessage && <div className="fixed inset-0 z-40 grid place-items-center bg-stone-950/35 px-4 backdrop-blur-sm"><section className="w-full max-w-sm rounded-2xl border border-stone-200 bg-white p-5 shadow-2xl"><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">Forward message</h2><p className="mt-1 text-sm text-stone-500">Choose a conversation.</p></div><button className="grid size-8 place-items-center rounded-full bg-stone-100" onClick={() => setForwardingMessage(null)} type="button">×</button></div><div className="mt-4 max-h-72 space-y-1 overflow-y-auto">{conversations.filter(({ id, blocked }) => id !== activeConversation.id && !blocked).map((conversation) => <button className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left hover:bg-stone-100" key={conversation.id} onClick={() => void forwardTo(conversation.id)} type="button"><Avatar className="size-9 text-sm" name={conversation.name} url={conversation.avatarUrl} /><span className="truncate text-sm font-medium">{conversation.name}</span></button>)}</div></section></div>}
      {removeConversationOpen && <div className="fixed inset-0 z-40 grid place-items-center bg-stone-950/35 px-4 backdrop-blur-sm"><section className="w-full max-w-sm rounded-2xl border border-stone-200 bg-white p-5 shadow-2xl"><h2 className="text-lg font-semibold">Remove conversation?</h2><p className="mt-2 text-sm leading-5 text-stone-500">This removes the chat from your account. The other person keeps their copy.</p><div className="mt-5 grid grid-cols-2 gap-2"><button className="rounded-lg border border-stone-200 px-4 py-2.5 text-sm font-medium" onClick={() => setRemoveConversationOpen(false)} type="button">Cancel</button><button className="rounded-lg bg-stone-900 px-4 py-2.5 text-sm font-medium text-white" onClick={() => void confirmRemoveConversation()} type="button">Remove</button></div></section></div>}

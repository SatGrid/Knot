...
export function ChatShell({
  ...
  return (
    <main className={`${darkMode ? "knot-dark" : ""} knot-app h-dvh overflow-hidden p-0 text-slate-900 sm:p-5`}>
      ...
      {viewingImage && <div className="fixed inset-0 z-50 flex flex-col bg-black/65 text-white backdrop-blur-xl" onMouseDown={(event) => { if (event.target === event.currentTarget) setViewingImage(undefined); }}><header className="flex h-16 shrink-0 items-center justify-between px-4 sm:px-6"><button className="rounded-lg px-3 py-2 text-sm font-medium hover:bg-white/10" onClick={() => setViewingImage(undefined)} type="button">← Back</button><p className="max-w-[60%] truncate text-sm text-stone-300">{viewingImage.name}</p><button aria-label="Close image" className="grid size-10 place-items-center rounded-full text-2xl hover:bg-white/10" onClick={() => setViewingImage(undefined)} type="button">×</button></header><div className="relative min-h-0 flex-1 p-4 sm:p-8"><Image alt={viewingImage.name} className="object-contain drop-shadow-2xl" fill priority sizes="100vw" src={viewingImage.url} unoptimized /></div></div>}
